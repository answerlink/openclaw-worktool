...
export async function postBridge(baseUrl: string, robotId: string, list: Array<Record<string, unknown>>) {
...
export function getWorktoolConfig(cfg: any) {
...
export function resolveWorktoolAccount(cfg: any, accountId?: string): WorktoolAccount {
  const c = getWorktoolConfig(cfg);
...
async function sendBridgeText(account: WorktoolAccount, to: string, text: string) {
...
async function sendBridgeMedia(account: WorktoolAccount, to: string, text: string, mediaUrl: string) {
  if (!account.robotId || !account.bridgeBaseUrl) {
    throw new Error("worktool channel not configured: require robotId + bridgeBaseUrl");
  }
