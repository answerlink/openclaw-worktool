import { createReplyPrefixContext, type ClawdbotConfig, type RuntimeEnv } from "openclaw/plugin-sdk";
...
function buildSyntheticMessageId(parts: Array<string | number>): string {
  return parts
    .map((p) => String(p || "").trim())
    .filter(Boolean)
    .join("|")
...
export function normalizeInboundWebhook(payload: unknown): InboundMessage | null {
  if (!payload || typeof payload !== "object") return null;
  const body = payload as Record<string, unknown>;

  // 0) WorkTool webhook schema (spoken/rawSpoken/receivedName/groupName/roomType/...)
  ...
  if (spoken || rawSpoken || receivedName || groupName || groupRemark || roomType) {
    const isGroup = (() => {
      // roomType has higher priority when provided by upstream:
      // 1=external group, 3=internal group, 2=external contact, 4=internal contact.
  ...
  // 1) Feishu-like envelope (recommended)
  ...
  if (event && message && sender) {
    const senderIdObj = sender.sender_id && typeof sender.sender_id === "object"
...
export async function handleInboundWebhook(params: {
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    Provider: "worktool" as const,
    Surface: "worktool" as const,
    MessageSid: msg.messageId,
    Timestamp: msg.timestamp,
    WasMentioned: wasMentioned,
    OriginatingChannel: "worktool" as const,
  ...
  const { dispatcher, replyOptions, markDispatchIdle } = core.channel.reply.createReplyDispatcherWithTyping({
    ...
    deliver: async (payloadChunk: { text?: string }) => {
  ...
  runtime?.log?.(
    `worktool[${account.accountId}]: inbound dispatched message=${msg.messageId} session=${route.sessionKey}`,
  );
