...
function readQueryToken(req: http.IncomingMessage): string {
  const url = req.url || "/";
...
function deriveAdminPath(webhookPath: string): string {
  const p = webhookPath || "/wechat/webhook";
  if (p.endsWith("/webhook")) {
    return `${p.slice(0, -"/webhook".length)}/admin`;
  }
...
async function persistWorktoolConfig(params: {
  ...
  const section = cfg.channels.worktool as Record<string, any>;
    ...
    if (reqPath === adminTestPath && req.method === "POST") {
      ...
      try {
        ...
        void handleInboundWebhook({
          ...
          accountId,
          runtime,
        }).catch((err) => error(`worktool[${accountId}]: async admin test error: ${String(err)}`));
      } catch (err) {
        sendJson(res, 400, { ok: false, error: String(err) });
      }
      return;
